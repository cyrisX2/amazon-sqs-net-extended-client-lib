...
namespace Amazon.SQS.ExtendedClient
{
    ...
    public static class ExtendedSQSEvent
    {
        ...
        /// <returns></returns>
        public static async Task<List<Lambda.SQSEvents.SQSEvent.SQSMessage>> ProcessMessageAsync(IAmazonS3 s3Client,
            List<Lambda.SQSEvents.SQSEvent.SQSMessage> messages, CancellationToken cancellationToken = default(CancellationToken))
        {

            if (messages == null) throw new AmazonClientException("messages cannot be null");

            for (var i = 0; i < messages.Count; i++)
                messages[i] = await ProcessMessageAsync(s3Client, messages[i], cancellationToken)
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public static async Task<Lambda.SQSEvents.SQSEvent.SQSMessage> ProcessMessageAsync(IAmazonS3 s3Client,
            Lambda.SQSEvents.SQSEvent.SQSMessage message,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var originalMessageBody =
                await AmazonSQSExtendedClient.GetTextFromS3Async(s3Client, messageS3Pointer.S3BucketName, messageS3Pointer.S3Key,
                    cancellationToken).ConfigureAwait(false);
            message.Body = originalMessageBody;
            message.ReceiptHandle = AmazonSQSExtendedClient.EmbedS3PointerInReceiptHandle(message.ReceiptHandle,
